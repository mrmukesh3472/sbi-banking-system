...
public class CreditServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp)
        ...
        if (amount <= 0) {
            resp.sendRedirect("credit.html?status=invalid_amount");
            return;
        }
        ...
        try {
            ...
            if (!rs.next()) {
                con.close();
                resp.sendRedirect("credit.html?status=invalid_account");
                return;
            }
            ...
            history.setString(3, "CREDIT");
            history.setDouble(4, amount);
            history.setNull(5, Types.BIGINT);  // MySQL compatible
            history.executeUpdate();

            con.close();
            resp.sendRedirect("credit.html?status=success&amount=" + amount);

        } catch (Exception e) {
            e.printStackTrace();
            resp.sendRedirect("credit.html?status=error");
        }
    }
}
